...
def get_otp(length=6, digit=True, alpha=True, lowercase=True, uppercase=True):
    ...
    uprChars = "QWERTYUIOPLKJHGFAZXCVBNM"
    digs = "0123456789"
    chars = ""
    otp = ""
...
def random_password(length=8, special_chars=True, digits=True):
    spec_chars = ['!', '@', '#', '$', '%', '^', '&', '*']
    alpha = "QWERTYUIOPLKJHGFAZXCVBNMmnbvcxzasdfghjklpoiuytrewq"
...
def get_alphabetic_profile_img(char, filePath, imgName, charColor=None, bgColor=None):
    ...
    if all((c in chars) for c in imgName):
        ...
        raise OSError(
            "Invalid image name. Image name must contain characters including digits, alphabets, white space, dot, comma, ( ) [ ] { } _ + - =.")
    return filePath
...
def get_birthdate(startAge=None, endAge=None, _format="%d %b, %Y"):
    ...
    elif startAge != None or endAge != None:
...
def get_address():
    ...
    allAddrs = {i: [] for i in range(5, 12)}

    for addr in addrFile:
        for i in range(5, 12):
            if len(addr) > i and addr[i] != '':
                allAddrs[i].append(addr[i])

    for i in range(5, 12):
        if allAddrs[i]:
            full_addr.append(choice(allAddrs[i]))
        else:
